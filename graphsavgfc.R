...
fileNames <- c("fuel_consumption_car.csv",
               ...
               "average_fuel_efficiency_car.csv")

varNames <- c("Fuel consumption, cars",
              "Travel delay, cars",
              "Travel delay, trucks",
              "Average speed, cars",
              "Average fuel efficiency, cars")

outFileName <- "sim_data_graphs_avfc.pdf"
...
simLabeller <- function(var, value){
  ...
  if (var=="depvars") { 
    ...
    value[value=="af_car"] <- "Average fuel efficiency, cars"
  }
  return(value)
}

# load CSV files
for (depVar in 1:5) {  # load CSV files
